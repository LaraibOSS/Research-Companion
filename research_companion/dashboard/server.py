...
from research_companion.dashboard.page import PAGE_HTML
...
def _run_uvicorn_in_thread(app, port: int) -> None:
    ...
    for _ in range(20):
        ...
        time.sleep(0.1)
    print(
        "research-companion: warning: dashboard failed to start (port in use?)",
        file=sys.stderr,
    )


def create_app(bus: Bus, state: dict) -> FastAPI:
    app = FastAPI(title="research-companion dashboard")
