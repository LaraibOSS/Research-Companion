...
Usage
-----
    python -m research_companion.eval.novelty_openreview cases.jsonl [--out DIR]

Each line of ``cases.jsonl`` must be a JSON object::

    {"paper_id": "arxiv:2401.00001", "human_novelty": 4}

The paper must already be in the store with its text saved (``research-companion add``
+ ``research-companion build`` + text ingested).  ``human_novelty`` is a 1-5 score
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m research_companion.eval.novelty_openreview",
