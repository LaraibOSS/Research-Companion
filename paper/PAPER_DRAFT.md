...
- **C3 — An open, tested, reproducible system.** MIT-licensed; 868 tests (704 Python, 164 JavaScript) with all network and LLM calls behind injectable seams; zero-key offline demos; committed, seed-deterministic evaluation artifacts; and an evaluation harness for novelty-vs-human-reviewer agreement.
...
The CLI covers the full lifecycle (add/build/chat/discover/refcheck/review/rebuttal, plus lab serve/ingest, set-draft, align, ask, compare). `review --serve` opens a live dashboard: one lane per agent with status and latest finding, plus a scrolling findings feed (XSS-escaped; paper content is untrusted input). The final report is a single self-contained HTML file — lane cards, non-verified references with reasons, a claims table with verdicts and evidence-verification flags, confidence cards with bands, benchmark suggestions — plus a machine-readable JSON twin. Bundled offline demos (`examples/demo_offline.py`, `examples/demo_lab_offline.py`) reproduce the review pipeline and the Lab event stream with zero API keys and zero network, using the same injection seams as the test suite.

**The Research Lab.** `lab serve` opens a local web workspace over the same store. Papers arrive singly (arXiv ID, URL, or PDF path) or in bulk: `lab ingest <folder>` walks a directory of PDFs through the full pipeline, and every stage publishes typed events over Server-Sent Events, so the knowledge graph *visibly grows* in the browser as sections are parsed and entities extracted; per-file failures surface as retryable cards rather than buried log lines. Each paper's text is split into a section tree (deterministic heading heuristics with a guaranteed whole-text fallback; an optional LLM refinement whose proposed headings must match the text verbatim), and extraction tags every entity with its section — so the graph offers instant per-section subgraphs, and the *ask* box retrieves only the most relevant sections (embedding-free BM25 over section units) before any LLM call: the token-efficiency counterpart of verification-first design. When the researcher designates a *draft*, every other paper receives an alignment verdict — which draft sections it strengthens, challenges, or offers an alternative perspective on — with evidence quotes verified verbatim against the candidate paper's own text and a deterministic usefulness score with an uncertainty band (the confidence agent's construction). Library cards are color-coded by an explainable strength score (extraction completeness, citation health, alignment, recency; weights renormalized over available signals, every signal shown). Grounded answers cite tagged sources inline, and any quoted span that cannot be located verbatim in a source is flagged — the same honesty gradient as the review lanes. A pairwise *compare* view reports shared and unique concepts, methods, and datasets plus a side-by-side results table, deterministically. Screenshots from a live run are committed under `paper/figures/`.
...
**System verification.** 868 tests, TDD throughout: 704 Python tests with all LLM/network behind injectable seams (six-lane pipeline, Lab server endpoints + SSE, both CLIs, no network) plus 164 JavaScript tests for the Lab client's pure core. A committed event-stream fixture is the executable frontend/backend contract. Bad LLM JSON fails one lane cleanly; HTML/JS in paper-derived strings is escaped in report, dashboard, and Lab.
