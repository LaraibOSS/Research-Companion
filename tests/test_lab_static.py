...
def test_report_surfaces_claim_audit_without_overclaiming():
    ...
    rep = (STATIC_DIR / "js" / "views" / "report.js").read_text(encoding="utf-8")
    helper = (STATIC_DIR / "js" / "claimAuditHelpers.js").read_text(encoding="utf-8")
    api_js = (STATIC_DIR / "js" / "api.js").read_text(encoding="utf-8")
    css = (STATIC_DIR / "css" / "lab.css").read_text(encoding="utf-8")
    assert "runClaimAudit" in api_js and "/api/claim-audit" in api_js
    assert "report-audit-btn" in rep and "_runAudit" in rep
    assert "claimAuditBadge" in rep and "claimAuditSummaryLine" in rep
    # the advisory framing must travel with the result
    assert "CLAIM_AUDIT_DISCLAIMER" in rep
    assert "advisory" in helper and "can be wrong" in helper
    # inconclusive outcomes are muted, adverse is a warning -- distinct tones
    assert ".report-audit-badge.audit-muted" in css
    assert ".report-audit-badge.audit-warn" in css
...
def test_add_papers_gives_a_receipt_and_manual_fallback():
    """Adding papers is async; the UI must say what was queued, where it went,
    what was skipped as already-present, and what to do when a PDF cannot be
    downloaded."""
    bs = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    helper = (STATIC_DIR / "js" / "addReceipt.js").read_text(encoding="utf-8")
    css = (STATIC_DIR / "css" / "lab.css").read_text(encoding="utf-8")
    assert "addReceiptModel" in bs and "_addReceipt" in bs
    # duplicates are counted and reported, not silently ignored
    assert "skipped" in bs and "skipped" in helper
    # the manual-download fallback is stated
    assert "manually" in helper and "MANUAL_ADD_DISCLAIMER" in helper
    # dismissible, and points at the Library
    assert "brainstorm-receipt-close" in bs and "#/library" in bs
    assert ".brainstorm-add-receipt" in css
...
def test_directions_shows_provenance_and_ingest_state():
    """Research Directions explains itself: an intro, the real grounding counts,
    an ingest-in-progress notice, and a not-exhaustive disclaimer."""
    bs = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    helper = (STATIC_DIR / "js" / "directionsProvenance.js").read_text(encoding="utf-8")
    css = (STATIC_DIR / "css" / "lab.css").read_text(encoding="utf-8")
    # pure helpers used by the view
    assert "ingestStatus" in bs and "provenanceLine" in bs
    assert "DIRECTIONS_INTRO" in bs and "DIRECTIONS_DISCLAIMER" in bs
    # the view keeps the server's real grounding counts
    assert "_directionsGrounding" in bs and "data.grounding" in bs
    # ingest notice live-updates rather than going stale
    assert "_refreshIngestNotice" in bs
    # honesty copy
    assert "not exhaustive" in helper
    assert "manually" in helper
    assert ".brainstorm-directions-intro" in css
    assert ".brainstorm-ingest-notice" in css


def test_brainstorm_brief_panel():
    """The Brainstorm tab renders the grounded-bullet Brief panel: a generate
    button, the pure model + edit ops, citation chips, and a note affordance."""
    bs = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    api_js = (STATIC_DIR / "js" / "api.js").read_text(encoding="utf-8")
    css = (STATIC_DIR / "css" / "lab.css").read_text(encoding="utf-8")
    helper = STATIC_DIR / "js" / "briefHelpers.js"
    assert helper.exists(), "briefHelpers.js must exist"
    # api client + endpoint
    assert "/api/brief" in api_js
    assert "export const brief" in api_js
    # view uses the pure model + edit ops and the generate handler
    assert "briefModel" in bs and "_generateBrief" in bs
    for op in ("setBulletText", "deleteBullet", "addBullet", "deleteSection", "moveBullet"):
        assert op in bs, f"brief edit op {op} not wired"
    # editable + note-able + honesty caption
    assert "data-brief-edit" in bs and "data-brief-note" in bs
    assert "grounded in your papers, edit freely" in bs
    # notes reuse the shared record builder
    assert "buildNoteRecord" in bs and "saveNote" in api_js
    # styling present
    assert ".brief-section" in css and ".brief-cite" in css
...
def test_brainstorm_session_persistence():
    ...
    bs = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    api_js = (STATIC_DIR / "js" / "api.js").read_text(encoding="utf-8")
    helper = STATIC_DIR / "js" / "brainstormSessionHelpers.js"
    assert helper.exists(), "brainstormSessionHelpers.js must exist"
    # api client for both directions of the session endpoint
    assert "getBrainstormSession" in api_js and "putBrainstormSession" in api_js
    assert "/api/brainstorm/session" in api_js
    # the view hydrates on mount and persists after mutations
    assert "serializeSession" in bs and "hydrateSession" in bs
    assert "_hydrate" in bs and "_persist" in bs
    # the searched topic is restored into the box
    assert "value=\"${escapeHtml(_topic)}\"" in bs


def test_draft_outline_fallback_and_analyze_button():
    """Draft view falls back to the outline when no alignment exists and offers
    a one-click "Analyze this draft" that hits POST /api/draft/analyze."""
    draft_js = (STATIC_DIR / "js" / "views" / "draft.js").read_text(encoding="utf-8")
    helpers = (STATIC_DIR / "js" / "draftHelpers.js").read_text(encoding="utf-8")
    api_js = (STATIC_DIR / "js" / "api.js").read_text(encoding="utf-8")
    css = (STATIC_DIR / "css" / "lab.css").read_text(encoding="utf-8")
    # pure fallback model + its use in the view
    assert "draftSectionModel" in helpers and "fromOutline" in helpers
    assert "draftSectionModel" in draft_js and "getSections" in draft_js
    # one-click analyze: api client + endpoint + button wiring
    assert "analyzeDraft" in api_js and "/api/draft/analyze" in api_js
    assert "analyzeDraft" in draft_js and "draft-analyze-btn" in draft_js
    # button state derived from the real background job, not a stuck flag
    assert "activity" in draft_js
    assert ".draft-toolbar" in css and ".draft-analyze-btn" in css


def test_every_view_has_persistent_help():
    """The router attaches a persistent "?" help affordance to every view, with
    per-page copy (what it does / what it uses / AI disclaimer)."""
    router = (STATIC_DIR / "js" / "router.js").read_text(encoding="utf-8")
    panel = STATIC_DIR / "js" / "components" / "pageHelp.js"
    content = STATIC_DIR / "js" / "helpContent.js"
    css = (STATIC_DIR / "css" / "lab.css").read_text(encoding="utf-8")
    assert panel.exists() and content.exists()
    # attached centrally so no view can forget it
    assert "attachHelp" in router
    body = content.read_text(encoding="utf-8")
    # the honesty disclaimer must exist and mention verifying
    assert "AI_DISCLAIMER" in body and "verify" in body.lower()
    # every navigable tab has an entry
    for view in ("brainstorm", "library", "graph", "draft", "timeline",
                 "gaps", "report", "ask", "citations", "notes", "researches"):
        assert f"{view}:" in body, f"no help entry for {view}"
    assert ".rc-help-fab" in css and ".rc-help-panel" in css
...
# ---------------------------------------------------------------------------
# feat/brainstorm-novelty: Novelty Gate (per-direction "Check novelty")
# ---------------------------------------------------------------------------

def test_api_js_has_check_novelty_client():
    api_js = (STATIC_DIR / "js" / "api.js").read_text(encoding="utf-8")
    assert "/api/novelty" in api_js
    assert "export function checkNovelty(" in api_js or "export const checkNovelty" in api_js


def test_brainstorm_view_has_check_novelty_button_and_calls_api():
    text = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    assert "brainstorm-novelty-btn" in text
    assert "Check novelty" in text
    assert "api.checkNovelty(" in text
    assert "noveltyResultModel" in text


def test_brainstorm_view_has_novelty_map_and_retry_state():
    text = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    assert "_noveltyByDirectionId" in text
    assert "brainstorm-novelty-retry-btn" in text


# ---------------------------------------------------------------------------
# feat/brainstorm-scaffold: "Draft this direction" (Brainstorm 2d)
# ---------------------------------------------------------------------------

def test_api_js_has_scaffold_draft_client():
    api_js = (STATIC_DIR / "js" / "api.js").read_text(encoding="utf-8")
    assert "/api/directions/draft" in api_js
    assert "export function scaffoldDraft(" in api_js or "export const scaffoldDraft" in api_js


def test_brainstorm_view_has_draft_this_direction_button_and_calls_api():
    text = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    assert "brainstorm-scaffold-btn" in text
    assert "Draft this direction" in text
    assert "api.scaffoldDraft(" in text
    assert "scaffoldPanelModel" in text


def test_brainstorm_view_scaffold_draft_calls_api_inside_ensure_active_research():
    """The mutating scaffold call MUST be wrapped in ensureActiveResearch,
    mirroring _addOne/_addAll -- unlike the read-only _checkNovelty."""
    text = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    start = text.index("async function _scaffoldDraft")
    end = text.index("\n\nfunction _bindScaffoldPanelEvents", start)
    body = text[start:end]
    assert "ensureActiveResearch(async () => {" in body
    assert "api.scaffoldDraft(" in body


def test_brainstorm_view_has_scaffold_map_and_retry_state():
    text = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    assert "_scaffoldByDirectionId" in text
    assert "brainstorm-scaffold-retry-btn" in text


def test_brainstorm_view_has_open_draft_link():
    view = (STATIC_DIR / "js" / "views" / "brainstorm.js").read_text(encoding="utf-8")
    helpers = (STATIC_DIR / "js" / "scaffoldHelpers.js").read_text(encoding="utf-8")
    # The draft route is a single source-of-truth constant in the pure helper;
    # the view renders it (via the openDraftRoute model field) as the "Open draft" link.
