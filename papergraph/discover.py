...
class DiscoveredPaper:
    ...
    def add_cmd(self) -> str:
        """Suggested `papergraph add` command for this paper."""
        if self.arxiv_id:
            return f"papergraph add {self.arxiv_id}"
        if self.doi:
            return f"papergraph add {self.doi}"
        if self.s2_id:
            return f"papergraph add {self.s2_id}"
        return f"papergraph add {self.url}"
