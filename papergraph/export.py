"""Export the papergraph knowledge graph to portable formats.
...
def _load_data() -> tuple[Any, list[Any], dict[str, dict]]:
    ...
    from papergraph.store import list_papers, load_extraction
...
def _export_markdown(output_dir: Path) -> Path:
    ...
    index_lines: list[str] = ["# papergraph — Paper Index\n"]
...
def _export_json(output_dir: Path) -> Path:
    from papergraph.store import graph_json_path
