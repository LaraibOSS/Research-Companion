...
- **`tools.v1.json`** — the four **deterministic, key-free, zero-LLM-cost** tools
  (`verify_citation`, `ground_claim`, `citation_coverage`, `search_library`),
  shipped in v0.7.0. Safe to expose by default: they touch only the local store
  (plus, for `verify_citation`, read-only scholarly-metadata lookups) and never
  spend API money or require a key.

The costed, key-requiring tools (`ask_library`, `review_draft`) are deferred to a
future release behind an explicit budget gate — the "cost-and-keys boundary" — and
will ship as `tools.v2.json`.
